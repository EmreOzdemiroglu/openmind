...
case "$(uname)" in
Darwin)
    brew install git jansson meson ninja pkg-config ${extras:+fzf} ${lint:+llvm}
    exit 0
    ;;
FreeBSD)
    reject_lint FreeBSD
    ...
    as_root pkg install $assume_yes curl git jansson meson ninja pkgconf \
        python3 ${extras:+fzf}
    exit 0
    ;;
OpenBSD)
    ...
    as_root pkg_add -I curl git jansson meson ninja ${extras:+fzf}
    exit 0
    ;;
esac
...
*debian* | *ubuntu*)
    ...
    as_root apt-get install $assume_yes --no-install-recommends \
        build-essential git libcurl4-openssl-dev libjansson-dev \
        meson ninja-build pkg-config python3 ${extras:+fzf} \
        ${lint:+clang-format clang-tidy}
    ;;
*fedora* | *rhel* | *centos*)
    as_root dnf install $assume_yes gcc git make libcurl-devel jansson-devel \
    ...
    ;;
*suse*)
    as_root zypper install $assume_yes gcc git make libcurl-devel libjansson-devel \
        meson ninja pkgconf-pkg-config python3 ${extras:+fzf} ${lint:+clang-tools}
    ;;
*arch*)
    arch_pkgs="gcc git make curl jansson meson ninja pkgconf python ${extras:+fzf} ${lint:+clang}"
...
*alpine*)
    ...
    as_root apk add --no-cache \
        build-base git meson samurai curl-dev jansson-dev python3 ${extras:+fzf}
    ;;
*)
